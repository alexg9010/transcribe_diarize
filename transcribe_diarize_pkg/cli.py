...
def require_dependency(import_name: str, package_name: str):
    ...
    try:
        ...
        print(
            ...
            f"Run this script via `uv run transcribe_diarize.py ...`, "
            f"`transcribe-diarize ...`, or install {package_name}.",
            file=sys.stderr,
        )
...
def parse_speakers(speakers_str: str) -> dict[str, str]:
    ...
    for pair in parts:
        if "=" not in pair:
            print(
                f"Warning: ignoring invalid speaker mapping '{pair}' (expected KEY=VALUE)",
                file=sys.stderr,
            )
            continue
...
def load_prompt_template() -> str:
    """Load the repo prompt template when available, otherwise the packaged default."""
    repo_prompt_path = repo_root() / "prompt_summarize.txt"
    if repo_prompt_path.exists():
        return repo_prompt_path.read_text(encoding="utf-8")
    return (
        importlib.resources.files("transcribe_diarize_pkg")
        .joinpath("prompt_summarize.txt")
...
def repo_root() -> Path:
    configured_root = os.environ.get("TRANSCRIBE_DIARIZE_REPO_ROOT")
    if configured_root:
        return Path(configured_root).expanduser()
    return Path.home() / "git" / "transcribe_diarize"
...
def load_repo_env_values() -> dict[str, str]:
...
def resolve_hf_token(cli_token: str | None) -> str | None:
    ...
    if env_token:
        return env_token
    return load_repo_env_values().get("HF_TOKEN")
...
def main():
    ...
    parser.add_argument(
        "--hf_token",
        default=None,
        help="Hugging Face token. Defaults to HF_TOKEN, then the repo .env fallback.",
    )
    parser.add_argument(
        "--num_speakers",
        type=positive_int,
    ...
    if not hf_token:
        ...
        print("Provide --hf_token, set HF_TOKEN, or save HF_TOKEN in the repo .env file.")
        print(f"Repo .env fallback path: {repo_env_path()}")
    ...
    try:
        ...
        diarization = diarize(audio_path, hf_token=hf_token, num_speakers=args.num_speakers)
